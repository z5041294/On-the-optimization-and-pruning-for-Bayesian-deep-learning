...
class SGHMC_SPL(Optimizer):
        ...
        def __init__(self, params,N,betas=(0.1,1),eps1=5e-4,eps0=1e-1,soft_threshold=1e-2,hard_threshold=1e-3,warm_up=3000):
...
def sghmc(params: List[Tensor], grads: List[Tensor],soft_masks:List[Tensor],hard_masks:List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if epoch>warm_up and batch==0 and len(param.size())==2:
            ink1=(param.pow(2).mean((1))<soft_threshold)
            ink2=(param.pow(2).mean((1))<hard_threshold)
            soft_mask[ink1,:]=0
            soft_mask[~ink1,:]=1
            hard_mask[ink2,:]=0
            #hard_mask[~ink2,:]=1
