...
class SGHMC_SPL(Optimizer):
        ...
        def __init__(self, params,N,betas=(0.1,1),weight_decay_1=5e-4,weight_decay_0=1e-1,soft_threshold=1e-2,hard_threshold=1e-3,warm_up=3000):
            ...
            if not 0.0 <= weight_decay_1:
                raise ValueError("Invalid epsilon value: {}".format(weight_decay_1))
            if not 0.0 <= weight_decay_0:
                raise ValueError("Invalid epsilon value: {}".format(weight_decay_0))
            if not 0.0 < N:
            ...
            defaults = dict(N=N,betas=betas, weight_decay_1=weight_decay_1,weight_decay_0=weight_decay_0,soft_threshold=soft_threshold,hard_threshold=hard_threshold,warm_up=warm_up)
        ...
        def step(self,C,epoch,batch,T=1/50000,eta=0.1,closure=None):
            ...
            for group in self.param_groups:
                ...
                weight_decay_1=group['weight_decay_1']
                weight_decay_0=group['weight_decay_0']
                N=group['N']
                ...
                weight_decay_1,weight_decay_0,N,warm_up,
...
def sghmc(params: List[Tensor], grads: List[Tensor],soft_masks:List[Tensor],hard_masks:List[Tensor],
         ...
         weight_decay_1: float,weight_decay_0: float,N: int, warm_up:int,
    ...
    for i, param in enumerate(params):
        ...
        weight_decay=weight_decay_1*torch.ones_like(param, memory_format=torch.preserve_format)
        
        #EM update and pruning
        if epoch>warm_up and batch==0 and len(param.size())==2:
            ink1=(param.pow(2).mean((0))<soft_threshold)
            ink2=(param.pow(2).mean((0))<hard_threshold)
            soft_mask[:,ink1]=0
            soft_mask[:,~ink1]=1
            hard_mask[:,ink2]=0
            #hard_mask[:,~ink2]=1
                       
        weight_decay[soft_mask==0]=weight_decay_0

        # update step
        step_t += 1

        # Update rho
        denom1=(T**2)*(C**3)*weight_decay*tau+T*(C**1.5)*noise*grad-torch.reciprocal(tau*N)
        ...
        denom2 = grad+weight_decay*param
